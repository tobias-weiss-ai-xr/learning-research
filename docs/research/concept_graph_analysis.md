# Concept Graph Analysis

**Generated:** 42 nodes, 444 edges, 1 components, 1 communities (modularity -6539.121). Edges = co-occurrence.

## Top hubs (by PageRank)

- **framework** — PR 0.1815, degree 38, df 11749
- **survey** — PR 0.0856, degree 38, df 4654
- **dataset** — PR 0.0701, degree 37, df 3889
- **Machine Learning** — PR 0.0502, degree 16, df 6021
- **benchmark** — PR 0.0494, degree 35, df 2552
- **quantitative** — PR 0.0464, degree 38, df 1767
- **qualitative** — PR 0.0433, degree 37, df 1584
- **Cognitive Science** — PR 0.0399, degree 19, df 4345
- **Language** — PR 0.0344, degree 18, df 3878
- **Education** — PR 0.0290, degree 19, df 2924
- **Social Learning** — PR 0.0282, degree 18, df 2616
- **Memory Science** — PR 0.0221, degree 17, df 3055

## Bridge nodes (high betweenness — connect clusters)

- framework
- survey
- quantitative
- case study
- literature review
- dataset
- qualitative
- systematic review
- benchmark
- experiment

## Bridge edges (high edge-betweenness)

- framework ↔ grounded theory (weight 8, EB 3.9794)
- quantitative ↔ grounded theory (weight 6, EB 3.9794)
- survey ↔ grounded theory (weight 13, EB 3.9794)
- dataset ↔ grounded theory (weight 2, EB 3.8683)
- literature review ↔ grounded theory (weight 2, EB 3.8365)
- case study ↔ grounded theory (weight 2, EB 3.8365)
- qualitative ↔ grounded theory (weight 18, EB 3.8365)
- framework ↔ Neuromorphic (weight 175, EB 3.5082)
- quantitative ↔ Neuromorphic (weight 9, EB 3.5082)
- survey ↔ Neuromorphic (weight 180, EB 3.5082)

## Communities (clusters of related concepts)

### Cluster 1 (39 concepts)

Animal Learning, Behavioral, Cognitive Science, Collective, Creative, Developmental, Education, Educational Psychology, Emotion, Evolutionary, Health, Language


Generated by `tools/analyze_concept_graph.py` — do not edit by hand.